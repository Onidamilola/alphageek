...
const ScheduleModal = ({ visitSchedules }) => {
  ...
  const handleClick = (schedule) => {
    ...
    updatedVisited[schedule.schedule_id] = true; // Use schedule.schedule_id for specific visit tracking
    ...
    sessionStorage.setItem('schedule_id', JSON.stringify(scheduleId)); // Stringify for consistency
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button
              ...
              style={{
                width: '100%',
                padding: '10px 20px',
                backgroundColor: visited[visitSchedule.schedule_id] ? 'green' : '#007bff',
                color: '#fff',
                border: 'none',
                borderRadius: '5px',
                cursor: 'pointer',
            ...
            >
              {visited[visitSchedule.schedule_id] ? 'Visited' : 'Visit Schedule'}
            </button>
          </div>
        </div>
      ))}
